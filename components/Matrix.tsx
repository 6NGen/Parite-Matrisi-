...
const APP_VERSION = 'sürüm 9 · SKOR satırından Odaklı/Geniş geçişi';
            ...
            <tr className="score-row">
              <th className="row-head">
                ...
                <button
                  ...
                  onClick={() => setScoreMode((m) => (m === 'broad' ? 'focused' : 'broad'))}
                  title="Odaklı (sınıfa özel) ↔ Geniş (tüm satırlar) skorlama"
                >
                  {scoreMode === 'broad' ? 'geniş ⇄' : 'odaklı ⇄'}
                </button>
              </th>
              {FLAT_INDICES.map(({ idx }) => {
                const col = columnsByKey.get(idx.key);
